...
const { generateReportAnalysis } = require('../services/llmService');

const router = express.Router();
router.use(authMiddleware);

// GET /api/reports/summary/:version_id
router.get('/summary/:version_id', async (req, res) => {
  try {
    const db = getDb();
    ...
    res.json({
      success: true,
      data: {
        version, summary, byModule, byRootCause, fatalDefects, aiAnalysis,
        closeRate: parseFloat(closeRate),
        generatedAt: new Date().toISOString()
      }
    });
  } catch (err) {
    console.error('[Reports Summary] 失败:', err);
    res.status(500).json({ success: false, message: '生成报告失败', error: err.message });
  }
});
